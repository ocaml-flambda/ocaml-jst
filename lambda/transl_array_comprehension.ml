...
(** Utilities for combining CPS-encoded lambda terms.  List comprehensions are
    desugared "from the outside in" into a CPS representation: each clause and
    each iterator becomes a single piece of the loop that is waiting for its
    body, and these are composed together to produce the whole list
    comprehension. *)
...
let for_and_clause ~transl_exp ~loc ~scopes ~usage =
...
let clause ~transl_exp ~loc ~scopes = function
...
let clauses ~transl_exp ~loc ~scopes = function
  | [Texp_comp_for bindings] ->
