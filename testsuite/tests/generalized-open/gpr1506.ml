...
1 | include struct open struct type t = T end let x = T end
                   ^^^^^^^^^^^^^^^^^^^^^^^^^^
Error: The type t/336 introduced by this open appears in the signature
       Line 1, characters 46-47:
         The value x has no valid type if t/336 is hidden
|}];;
...
5 |       let x = T
6 |     end
Error: The type t/341 introduced by this open appears in the signature
       Line 7, characters 8-9:
         The value y has no valid type if t/341 is hidden
|}];;
...
4 |       type t = T
5 |     end
Error: The type t/346 introduced by this open appears in the signature
       Line 6, characters 8-9:
         The value y has no valid type if t/346 is hidden
|}]
