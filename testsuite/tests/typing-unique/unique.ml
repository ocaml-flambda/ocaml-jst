...
let update (unique_ r : record_update) =
  (* let r_ptr :int  = Obj.magic r in *)
  let unique_ x = { unique_ r with x = "foo" } in
  (* let x_ptr : int = Obj.magic x in *)
  (* assert (r_ptr = x_ptr); *)
  x.x
[%%expect{|
val update : unique_ record_update -> string = <fun>
|}]

let update2 = update { x = "bar" }
[%%expect{|
val update2 : string = "foo"
|}]
...
let gc_soundness_nobug (local_ unique_ p) f =
  local_ { unique_ p with x = f }
[%%expect{|
val gc_soundness_nobug : local_ unique_ point -> float -> local_ point =
  <fun>
|}]
