...
module DualSolver (DL : DualLattice) = struct
  ...
  let join_const a b = DL.from_dconst (DL.D.meet_const (DL.to_dconst a) (DL.to_dconst b))

  let meet_const a b = DL.from_dconst (DL.D.join_const (DL.to_dconst a) (DL.to_dconst b))
  ...
  let print ppf m =
    print' ~verbose:true ?label:None ppf m
end
...
module Linearity = struct
  module DL = struct
    ...
    let to_dconst = function
      | Once -> D.Unique
      | Many -> D.Shared

    let from_dconst = function
      | D.Unique -> Once
      | D.Shared -> Many
  end
  include DL
...
module Alloc = struct
  ...
  let min_mode : t = {locality = Locality.min_mode;
                  uniqueness = Uniqueness.min_mode;
                  linearity = Linearity.min_mode}

  let max_mode : t = {locality = Locality.max_mode;
  ...
  let submode {locality = loc1; uniqueness = u1; linearity = lin1 }
    ...
    match Locality.submode loc1 loc2 with
      | Ok () -> begin match Uniqueness.submode u1 u2 with
        | Ok () -> begin match Linearity.submode lin1 lin2 with
          | Ok () -> Ok ()
  ...
  let equate ({locality = loc1; uniqueness = u1; linearity = lin1}:t)
    ...
    | Ok () -> begin match Uniqueness.equate u1 u2 with
      | Ok () -> begin match Linearity.equate lin1 lin2 with
        | Ok () -> Ok ()
        | Error () -> Error `Linearity
      end
      | Error () -> Error `Uniqueness
  ...
  let of_uniqueness uniqueness =
    { locality = Locality.newvar (); uniqueness; linearity = Linearity.newvar ()}

  let of_locality locality =
    { locality; uniqueness = Uniqueness.newvar () ; linearity = Linearity.newvar ()}

  let of_linearity linearity =
    { locality = Locality.newvar (); uniqueness = Uniqueness.newvar () ;
    linearity}

  let with_locality locality t =
    {t with locality}

  let with_uniqueness uniqueness t =
    { t with uniqueness }

  let with_linearity linearity t =
    { t with linearity }
  ...
  let print' ?(verbose=true) ppf { locality; uniqueness; linearity } =
    Format.fprintf ppf
      "%a, %a, %a"
      (Locality.print' ~verbose ~label:"locality") locality
      (Uniqueness.print' ~verbose ~label:"uniqueness") uniqueness
      (Linearity.print' ~verbose ~label:"linearity") linearity
...
module Regionality = struct
  ...
  let const_of_locality
        ~(r_as_l : Locality.const)
        ~(r_as_g : Locality.const) =
  ...
  let of_const c =
    let r_as_l, r_as_g = match c with
  ...
  let print' ?(verbose = true) ?label ppf t =
    ...
    | None ->
        ...
        Format.fprintf ppf
          "r_as_l=%a r_as_g=%a"
          (Locality.print' ~verbose ?label:None) t.r_as_l
          (Locality.print' ~verbose ?label:None) t.r_as_g
...
module Value = struct
  ...
  let min_with_uniqueness u =
    { min_mode with uniqueness = u }

  let max_with_uniqueness u =
  ...
  let local_to_regional t = { t with locality = Regionality.local_to_regional t.locality}

  let regional_to_global t = { t with locality = Regionality.regional_to_global t.locality}

  let regional_to_local t = { t with locality = Regionality.regional_to_local t.locality}

  let global_to_regional t = { t with locality = Regionality.global_to_regional t.locality}

  let regional_to_global_alloc t = { t with locality = Regionality.regional_to_global_locality t.locality}

  let regional_to_local_alloc t = { t with locality = Regionality.regional_to_local_locality t.locality}

  let regional_to_global_locality t = Regionality.regional_to_global_locality t.locality

  let regional_to_local_locality t = Regionality.regional_to_local_locality t.locality
  ...
  let print' ?(verbose = true) ppf t =
    Format.fprintf ppf
      "%a, %a, %a"
      (Regionality.print' ~verbose ~label:"locality") t.locality
      (Uniqueness.print' ~verbose ~label:"uniqueness") t.uniqueness
      (Linearity.print' ~verbose ~label:"linearity") t.linearity
