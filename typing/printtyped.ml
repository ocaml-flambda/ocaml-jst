...
let typevar_layout ?(print_quote = true) ppf (v, l) =
  let pptv =
    if print_quote
...
let typevars ppf vs =
  List.iter (typevar_layout ppf) vs
;;
...
let var_layout ppf (v, l) =
  match l with
  | None -> fprintf ppf " '%s" v
  | Some lay -> fprintf ppf " ('%s : %a)" v Pprintast.layout_annotation lay
...
let rec core_type i ppf x =
  ...
  | Ttyp_poly (sl, ct) ->
      line i ppf "Ttyp_poly%a\n"
        (fun ppf -> List.iter (var_layout ppf)) sl;
      core_type i ppf ct;
