  ...
  let mk {pld_name=name;pld_mutable=mut;pld_type=arg;pld_loc=loc;
    ...
    Builtin_attributes.warning_scope attrs
      (fun () ->
         ...
         let cty = transl_simple_type env closed (Global, Shared) arg in
...
let transl_constructor_arguments env closed = function
  | Pcstr_tuple l ->
      let l = List.map (transl_simple_type env closed (Global, Shared)) l in
...
let make_constructor env type_path type_params sargs sret_type =
  ...
  | Some sret_type ->
      ...
      in
      let tret_type = transl_simple_type env false (Global, Shared) sret_type in
...
let transl_declaration env sdecl (id, uid) =
  ...
  let cstrs = List.map
    (fun (sty, sty', loc) ->
      transl_simple_type env false (Global, Shared) sty,
      transl_simple_type env false (Global, Shared) sty', loc)
    sdecl.ptype_cstrs
  in
  ...
  let (tkind, kind) =
    ...
    let (tman, man) = match sdecl.ptype_manifest with
      ...
      | Some sty ->
        ...
        let cty = transl_simple_type env no_row (Global, Shared) sty in
...
let transl_with_constraint id row_path ~sig_env ~sig_decl ~outer_env sdecl =
  ...
  let constraints =
    List.map (fun (ty, ty', loc) ->
      let cty = transl_simple_type env false (Global, Shared) ty in
      let cty' = transl_simple_type env false (Global, Shared) ty' in
