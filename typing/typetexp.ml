...
let rec transl_type env policy locality styp =
  Builtin_attributes.warning_scope styp.ptyp_attributes
    (fun () -> transl_type_aux env policy locality styp)

and transl_type_aux env policy locality styp =
  let loc = styp.ptyp_loc in
  ...
  | Ptyp_arrow _ ->
      ...
      let rec loop locality_acc uniqueness_acc args =
        match args with
        | (l, arr, arg_mode, arg) :: rest ->
          let arr =
            match arr with
            | Call_once -> Mode.Uniqueness.Unique
            | Call_many -> uniqueness_acc in
          let arg_cty = transl_type env policy (fst arg_mode) arg in
          let locality_acc = Mode.Locality.join_const locality_acc (fst arg_mode) in
          let uniqueness_acc = Mode.Uniqueness.meet_const uniqueness_acc (snd arg_mode) in
          let ret_mode =
            match rest with
            | [] -> ret_mode
          ...
          let arr_mode = Mode.Uniqueness.of_const arr in
          let ret_mode = Mode.Alloc.of_const ret_mode in
          let ty =
            newty
              (Tarrow((l,arg_mode,arr_mode,ret_mode), arg_ty, ret_cty.ctyp_type, Cok))
          in
          let arr_mode, _ = Mode.Uniqueness.newvar_below arr_mode in
          ctyp (Ttyp_arrow (l, arr_mode, arg_cty, ret_cty)) ty
      ...
      if List.length stl <> decl.type_arity then
        raise(Error(styp.ptyp_loc, env,
                    Type_arity_mismatch(lid.txt, decl.type_arity,
                                        List.length stl)));
      let args = List.map (transl_type env policy Mode.Alloc.Global) stl in
  ...
  | Ptyp_alias(st, alias) ->
      let cty =
        try
          ...
          in
          let ty = transl_type env policy locality st in
        ...
        | Rtag (l, c, stl) ->
            ...
            let tl =
              Builtin_attributes.warning_scope rf_attributes
                (fun () ->
                   List.map (transl_type env policy Mode.Alloc.Global) stl)
            in
            ...
            add_typed_field styp.ptyp_loc l.txt f;
              Ttag (l,c,tl)
        | Rinherit sty ->
          let cty = transl_type env policy Mode.Alloc.Global sty in
  ...
  | Ptyp_poly(vars, st) ->
      ...
      let cty = transl_type env policy locality st in
      let ty = cty.ctyp_type in
      univars := old_univars;
      end_def();
      generalize ty;
  ...
  | Ptyp_package (p, l) ->
      ...
      let ptys = List.map (fun (s, pty) ->
                             s, transl_type env policy Mode.Alloc.Global pty
                          ) l in
    ...
    | Otag (s, ty1) -> begin
        let ty1 =
          Builtin_attributes.warning_scope of_attributes
            (fun () -> transl_poly_type env policy Mode.Alloc.Global ty1)
        in
...
let transl_simple_type env fixed locality styp =
  univars := []; used_variables := TyVarMap.empty;
  let typ = transl_type env (if fixed then Fixed else Extensible) locality styp in
...
let transl_simple_type_univars env styp =
  ...
  begin_def ();
  let typ = transl_type env Univars Mode.Alloc.Global styp in
...
let transl_type_scheme env styp =
  ...
  begin_def();
  let typ = transl_simple_type env false Mode.Alloc.Global styp in
  end_def();
  generalize typ.ctyp_type;
  typ
