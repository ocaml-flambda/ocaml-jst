...
let complete_tags constructors tags =
...
let pats_of_type ?(always=false) env ty mode =
  ...
  | Tconstr (path, _, _) ->
      ...
      | Type_variant (cl, _) when always || List.length cl <= 1 ||
...
let complete_constrs constr all_tags =
  ...
  let unseen = complete_tags (c.cstr_consts+c.cstr_nonconsts) all_tags in
...
| Tpat_construct(_, {cstr_tag=Ordinary _},ps)
  ->
    let path = get_constructor_type_path p.pat_type p.pat_env in
    List.fold_left
...
| Tpat_construct (_, {cstr_tag=Extension _}, ps)->
