...
let virtual_method val_env meths self_type lab priv sty loc =
  ...
  let cty = transl_simple_type val_env false (Global, Unique) sty in
...
and class_type_field_aux env self_type meths
  ...
  | Pctf_val ({txt=lab}, mut, virt, sty) ->
      let cty = transl_simple_type env false (Global, Unique) sty in
      let ty = cty.ctyp_type in
...
and class_signature env {pcsig_self=sty; pcsig_fields=sign} =
  ...
  let self_cty = transl_simple_type env false (Global, Unique) sty in
...
and class_field_aux self_loc cl_num self_type meths vars
  ...
  | Pcf_val (lab, mut, Cfk_virtual styp) ->
      ...
      let cty = Typetexp.transl_simple_type val_env false (Global, Unique) styp in
      let ty = cty.ctyp_type in
        ...
        Pexp_poly (sbody, sty) ->
          begin match sty with None -> ()
                | Some sty ->
                    let sty = Ast_helper.Typ.force_poly sty in
                    let cty' =
                      Typetexp.transl_simple_type val_env false (Global, Unique) sty
                    in
                    let ty' = cty'.ctyp_type in
...
and class_expr_aux cl_num val_env met_env scl =
  match scl.pcl_desc with
    Pcl_constr (lid, styl) ->
      ...
      let tyl = List.map
          (fun sty -> transl_simple_type val_env false (Global, Unique) sty)
          styl
      in
      let (params, clty) =
