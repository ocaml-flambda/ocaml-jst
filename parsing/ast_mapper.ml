...
let map_loc2 sub f {loc; txt} = {loc = sub.location sub loc; txt = f sub txt}
...
module T = struct
  ...
  let type_vars_layouts sub (tvls : type_vars_layouts) =
    List.map (map_opt (map_loc2 sub sub.layout_annotation)) tvls
  ...
  let map sub {ptyp_desc = desc; ptyp_loc = loc; ptyp_attributes = attrs} =
    ...
    | Ptyp_layout (t, lay) -> layout ~loc ~attrs
                                (sub.typ sub t)
                                (map_loc2 sub sub.layout_annotation lay)
  ...
  let map sub {pexp_loc = loc; pexp_desc = desc; pexp_attributes = attrs} =
    ...
    | Pexp_newtype (s, e, l) ->
        newtype ~loc ~attrs (map_loc sub s) (sub.expr sub e)
          (map_opt (map_loc2 sub sub.layout_annotation) l)
